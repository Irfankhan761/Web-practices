...
// Q26:Flatten Deeply Nested Array (Medium)
...
var flat = function (arr, n) {
  ...
  // Recursive case: reduce the array by 1 level of depth
  return arr.reduce((acc, val) => {
    if (Array.isArray(val)) {
      // If the element is an array, flatten it one level less
      acc.push(...flat(val, n - 1));
    } else {
      // If the element is not an array, just push it to the accumulator
      acc.push(val);
    }
    return acc;
  }, []);
