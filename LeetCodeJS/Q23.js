...
Array.prototype.groupBy = function (fn) {
  ...
  for (let item of this) {
    const key = fn(item); // Get the key by applying the function on the item
    if (!grouped[key]) {
      grouped[key] = []; // Initialize an empty array if the key doesn't exist
    }
    grouped[key].push(item); // Push the item into the correct group
  }
